...
export async function myData(req, res){
    ...
    try {
        const user = req.user;
        return res.status(https_codes.SUCCESS).json({data: user});
...
export async function updateUser(req,res){
    try {
        ...
        user.updateOne({name, email}).select('-password').then((updatedUser)=>{
            return res.status(https_codes.SUCCESS).json({data:updatedUser});
        }).catch((error)=>{
            console.error('error from updateUser\'s updateOne function:', error);
            return res.status(https_codes.SERVER_ERROR).json({error: "error on updating user."});
        })
    } catch (error) {
